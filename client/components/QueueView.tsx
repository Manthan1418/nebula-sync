...
function QueueContent({ queue, history, currentTrack, isHost, canPlayLocally, removeFromQueue, clearQueue, selectTrack, onClose }: {
  queue: any[]; history: any[]; currentTrack: any; isHost: boolean;
  canPlayLocally: boolean; removeFromQueue: (id: string) => void;
  clearQueue: () => void; selectTrack: (t: any) => void; onClose: () => void;
}) {
  return (
    <div className="p-2">
      {currentTrack && (
      ...
      <div className="flex items-center justify-between px-3 py-2">
        ...
        {isHost && queue.length > 0 && (
          <button onClick={clearQueue}
            className="p-1 rounded-lg hover:bg-surface-container transition-colors text-on-surface-variant hover:text-error">
            <Trash2 size={12} />
          </button>
        )}
      </div>
...
export function QueueView({ onClose, inline }: { onClose: () => void; inline?: boolean }) {
  ...
  if (inline) {
    return (
      <QueueContent
        queue={queue} history={history} currentTrack={currentTrack}
        isHost={isHost} canPlayLocally={canPlayLocally}
        removeFromQueue={removeFromQueue} clearQueue={clearQueue} selectTrack={selectTrack}
        onClose={onClose}
      />
    )
  }
